...
ZSH_THEME_GIT_PROMPT_DIRTY="%F{blue}*%f%{$terminfo[bold]$fg[yellow]%}"
...
PROMPT='%{$terminfo[bold]$fg[white]%}%n%{$reset_color%}%{$terminfo[bold]$fg[white]%} at %{$reset_color%}%{$terminfo[bold]$fg[green]%}%m%{$reset_color%}%{$terminfo[bold]$fg[white]%} in %{$reset_color%}%{$terminfo[bold]$fg[red]%}%0~$(git_branch)%{$terminfo[bold]$fg[white]%} ⇒%{$reset_color%}  '
